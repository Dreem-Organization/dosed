...
class DOSED1(BaseNet):
    ...
    def __init__(
        ...
        self.number_of_default_events = self.window_size * self.rho / (2 ** self.k_max)
        ...
        self.localizations = nn.Conv2d(
            ...
            kernel_size=(self.number_of_channels, 3),
            padding=(0, 1),
        )
        ...
        self.localizations_default = torch.Tensor([
            [((2 ** self.k_max)) * (0.5 + i) / (self.rho * self.window_size),
             self.duration / self.window_size] for i in range(int(self.number_of_default_events))
        ]
        )
    ...
    def forward(self, x):
        ...
        localizations = self.localizations(x).view(
            batch,
            2 * self.rho,
            int(self.window_size / (2 ** self.k_max))
